...
- **UnraidDeck Cloud**: native **WebDAV** (`http://IP:8787/dav` — sync from any OS/client), a **Photos tab** with timeline and share thumbnails, and **public share links** with expiry/password/download counter. All built in, no extra containers (requires the `/mnt → /unraid` mount).
...
| `DUCKDNS_DOMAIN` + `DUCKDNS_TOKEN` | — | with `HTTPS=true`: a **real Let's Encrypt certificate** for `<name>.duckdns.org` via DNS-01 (no ports to open, auto-renewal) — no "Not Secure" warnings |
| `TZ` | `Europe/Rome` | |
