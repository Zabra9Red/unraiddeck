...
- **Consumo elettrico dall'UPS in tempo quasi reale** (poll ogni 10 s, `POLL_UPS` per cambiare): potenza assorbita misurata via `ups.realpower` o stimata da potenza nominale × carico, integrata in kWh con grafico orario e **storico per giorno / settimana / mese / anno, costi inclusi**. Il **costo in €** usa il prezzo €/kWh della tua bolletta (preset indicativi dei principali fornitori italiani — Enel, Eni Plenitude, Edison, A2A, Iren, Sorgenia, Octopus — o valore manuale). I dati sono **salvati su disco in `/config`** (SQLite): riavvii di container o server non li cancellano; retention 2 anni.
